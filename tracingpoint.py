...
class MonApp(CTk.CTk):
    ...
    def create_interface(self):
        self.main_frame = CTk.CTkFrame(
            self, width=760, height=560, corner_radius=20, fg_color=("gray85", "gray20")
        )
        ...
        self.titre_params = CTk.CTkLabel(
            self.params_frame, text="Paramètres de tracé", font=self.fonts["button"]
        )
        ...
        self.x_min = CTk.CTkSlider(
            self.x_frame, from_=-500, to=500, number_of_steps=200
        )
        self.x_min.set(-100)
        self.x_min.pack(pady=2)
        CTk.CTkLabel(self.x_frame, text="Min", font=self.fonts["label"]).pack()

        self.x_max = CTk.CTkSlider(
            self.x_frame, from_=-500, to=500, number_of_steps=200
        )
        self.x_max.set(100)
    ...
    def tracer(self):
        ...
        try:
            nouvelle_fonction = preparation_fonction(fonction)
            print(nouvelle_fonction)

            X = linspace(x_min, x_max, 110000)
            Y = []

            points_fonction = 0
            for x in X:
                try:
                    y_val = calcul_image(nouvelle_fonction, x)
                    Y.append(y_val)
                    if not isnan(y_val):
                        points_fonction += 1
                except:
            ...
            if len(segments) > 0:
                ...
                plt.plot(
                    X, Y, linewidth=2.5, label=f"f(x) = {fonction}", color="#1f77b4"
                )
            plt.xlim(-10, 10)
            plt.ylim(-7, 7)
            ...
            plt.title(
                f"Fonction f(x) = {fonction}", fontsize=16, fontweight="bold", pad=20
            )
            plt.legend(fontsize=12, framealpha=0.9)
            plt.tight_layout()

            if fonction not in self.historique_fonctions:
                self.historique_fonctions.append(fonction)
...
def preparation_fonction(fonction):
...
def calcul_image(fonction, x):
    y_val = eval(fonction, globals(), {"x": x})
    return nan if isinf(y_val) or isnan(y_val) else y_val


def creation_segments(X, Y):
    segments = []
    segment_x = []
    segment_y = []
